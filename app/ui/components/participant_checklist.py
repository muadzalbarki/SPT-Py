...
class ParticipantChecklist(QWidget):
    ...
    def _setup_ui(self):
        ...
        self.btn_select_all = QPushButton("Semua")
        self.btn_select_all.clicked.connect(self.select_all)
        count_bar.addWidget(self.btn_select_all)
    ...
    def load_participants(self):
        ...
        for cat in self._category_order:
            ...
            def _sort_key(p):
                j = p.jabatan.strip() if p.jabatan else ""
                return (_pos_rank.get(j.split()[0] if j else "", 3), j)
