...
class TemplateEngine:
    ...
    def detect_placeholders(self) -> list[str]:
        with zipfile.ZipFile(str(self._template_path), 'r') as z:
            ...
            self._merge_text_runs(tree)
            xml_str = etree.tostring(tree, encoding='unicode')
            return self._extract_placeholders_balanced(xml_str)

    def _merge_text_runs(self, tree):
        ns = 'http://schemas.openxmlformats.org/wordprocessingml/2006/main'
        for p in tree.findall(f'.//{{{ns}}}p'):
            t_elements = p.findall(f'.//{{{ns}}}t')
            if len(t_elements) <= 1:
                continue
            merged = ''.join(t.text or '' for t in t_elements)
            for t in t_elements:
                t.text = None
            t_elements[0].text = merged
